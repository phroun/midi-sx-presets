...
class MidiPresetService:
    ...
    def __init__(self, config_dir=None):
        ...
        self.cc_names = self._load_or_create_cc_names()
        self.cc_name_to_num = {v: k for k, v in self.cc_names.items()}
        ...
        self.tracked_ccs = {}   # cc_number -> value
        self.tracked_pc = None  # last program-change number (or None)
    ...
    def _load_or_create_cc_names(self):
        path = self.config_dir / CC_NAMES_FILENAME
        if path.exists():
            data = _yaml_load(path)
            return data.get("cc_names", {})
        example = {
            "cc_names": {
                1: "modulation",
                7: "volume",
        ...
        _yaml_dump(example, path)
        return example["cc_names"]
    ...
    def _preset_filename(note):
        """Return the filename for a given note number, e.g. 'preset-060.yaml'."""
    ...
    def _load_presets(self):
        ...
        for path in sorted(pdir.glob("preset-*.yaml")):
            stem = path.stem  # e.g. "preset-060"
            try:
                note = int(stem.split("-", 1)[1])
    ...
    def _save_preset_to_disk(self, note):
        """Write a single preset file: presets/preset-NNN.yaml"""
        pdir = self._presets_dir()
        pdir.mkdir(parents=True, exist_ok=True)
        path = pdir / self._preset_filename(note)
        _yaml_dump(self.presets[note], path)

    # -- CC name <-> number ---------------------------------------------------

    def _cc_num_to_name(self, cc_num):
        """Return the human-readable name for a CC, or the raw number."""
        return self.cc_names.get(cc_num, cc_num)

    def _cc_name_to_number(self, name):
        """Resolve a CC name (or raw int) back to a CC number."""
        if isinstance(name, int):
            return name
        if name in self.cc_name_to_num:
            return self.cc_name_to_num[name]
        # Fall back: try parsing as int (handles presets saved with raw numbers)
        try:
            return int(name)
    ...
    def _is_ours(self, data):
        """True if the SysEx data matches our manufacturer + device ID."""
        return (len(data) >= 3
    ...
    def _cmd_start_record(self):
        ...
        self.load_mode = False
        self.tracked_ccs = {}
        self.tracked_pc = None
        self.last_note = None
        _log("REC", "Recording started — send CCs/PCs, then a note-on to tag the preset.")
    ...
    def _cmd_save_preset(self):
        ...
        existing = self.presets.get(preset_key, {})
        if existing.get("read_only", False):
            _log("DENY", f"Preset {preset_key} is read-only — save rejected.")
            self.recording = False
            return

        # Build CC dict using human-readable names where possible
        cc_data = {}
        for cc_num, value in self.tracked_ccs.items():
            key = self._cc_num_to_name(cc_num)
            cc_data[key] = value
        ...
        if self.tracked_pc is not None:
            preset["program_change"] = self.tracked_pc
        if cc_data:
            preset["cc_values"] = cc_data

        self.presets[preset_key] = preset
        self._save_preset_to_disk(preset_key)

        cc_count = len(cc_data)
        pc_str = f", PC={self.tracked_pc}" if self.tracked_pc is not None else ""
        _log("SAVE", f"Preset {preset_key} saved ({cc_count} CCs{pc_str}).")
        self.recording = False
    ...
    def _recall_preset(self, note):
        ...
        for cc_name, value in preset.get("cc_values", {}).items():
            ...
            if cc_num is None:
                _log("WARN", f"Cannot resolve CC name '{cc_name}' — skipping.")
                continue
            self.midi_out.send(
    ...
    def _handle_message(self, msg):
        ...
        if self.recording:
            if msg.type == "control_change":
                self.tracked_ccs[msg.control] = msg.value
                _log("CC", f"{self._cc_num_to_name(msg.control)}({msg.control}) = {msg.value}")
            elif msg.type == "program_change":
                self.tracked_pc = msg.program
    ...
    def run(self):
        ...
        _log("INIT", f"CC names    : {len(self.cc_names)} mappings")
        print()
        mid = self.manufacturer_id
        dev = self.device_id
        print("SysEx commands (hex bytes including F0/F7):")
        print(f"  Start Record : F0 {mid:02X} {dev:02X} {CMD_START_RECORD:02X} F7")
...
def main():
    ...
    # Graceful shutdown on SIGTERM (useful when running as a background service)
